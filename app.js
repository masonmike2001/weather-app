...
async function fetchData(location) {
  ...
  await fetch(
    "https://api.openweathermap.org/data/2.5/weather?q=" +
      location +
      "&appid=0c1ce888b5b00b8dcae362794387f5bd",
    { mode: "cors" }
  )
    .then(
    ...
    .then(function (response) {
      ...
      for (let i = 0; i < response["list"].length; i++) {
        if (response["list"][i]["dt_txt"].slice(0, 10) != date) {
          ...
          data.dailyWeather[day] = weather
            .sort(
              (a, b) =>
                weather.filter((v) => v === a).length -
                weather.filter((v) => v === b).length
            )
            .pop();
...
function setDates() {
  ...
  for (let j = 0; j < 5; j++) {
    weekDOM[j].textContent = new Date(
      week[j].getFullYear(),
      week[j].getMonth(),
      week[j].getDate()
    );
...
function updateDOM(data) {
  ...
  document.querySelector("#sunrise-value").textContent = document
    .querySelector("#sunrise-value")
    .textContent.substring(16, 25);
  document.querySelector("#sunset-value").textContent = document
    .querySelector("#sunset-value")
    .textContent.substring(16, 25);
}
...
function changeBackground(data) {
  ...
  if (data.dailyWeather[0] === "Clear") {
    ...
    document.querySelector("source").setAttribute("src", "./assets/clouds.mp4");
  } else if (data.dailyWeather[0] === "Rain") {
    document.querySelector("source").setAttribute("src", "./assets/rain.mp4");
  } else if (data.dailyWeather[0] === "Snow") {
    document.querySelector("source").setAttribute("src", "./assets/snow.mp4");
  }
